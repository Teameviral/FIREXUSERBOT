...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        eviral = await event.client.get_entity(event.sender_id)
        if eviral.bot or eviral.verified or eviral.support:
            return

        eviralm = f"[{get_display_name(eviral)}](tg://user?id={eviral.id})"
