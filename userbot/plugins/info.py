...
<b>⚡ <a href='https://t.me/FirexSupport'>From DataBase of FIREX</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
async def info(event):
    if event.fwd_from:
        return
    eviral = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await eviral.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(eviral, "`An unexpected error has occurred.`")
    return
