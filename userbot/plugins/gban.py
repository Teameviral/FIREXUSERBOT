...
Eviral = str(ALIVE_NAME) if ALIVE_NAME else "eviral User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        FIREX = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
        FIREX = await edit_or_reply(ids, "`Ok! Gban ho rha h intzarrr karo....`")
    ...
    try:
        ...
        return await FIREX.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        if user.id == 2082798662:
            return await FIREX.edit(f"`How dare u trying to Gban my master`")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await FIREX.edit(f"Gbaning ho rha h 😂\n\nTotal Chats :- `{a}`")
            except:
                b += 1
    else:
        await FIREX.edit(f"`Either reply to a user or gib me user id/name`")
    try:
        if gmute(user.id) is False:
            return await FIREX.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        FIREX = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        FIREX = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await FIREX.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    try:
        ...
        return await FIREX.edit("**Som3ting W3nt Wr0ng**")
    if user:
        if user.id == 2082798662:
            return await FIREX.edit(
        ...
        await FIREX.edit("**Reply to a user**")
    try:
        if ungmute(user.id) is False:
            return await FIREX.edit("**Error! User already ungbanned.**")
    except:
        pass
    return await FIREX.edit(
...
@borg.on(ChatAction)
async def handler(eviral):
    if eviral.user_joined or eviral.user_added:
        try:
            from userbot.plugins.sql_helper.gmute_sql import is_gmuted

            guser = await eviral.get_user()
        ...
        if gmuted:
            for i in gmuted:
                if i.sender == str(guser.id):
                    chat = await eviral.get_chat()
                    admin = chat.admin_rights
                    creator = chat.creator
                    if admin or creator:
                        try:
                            await client.edit_permissions(
                                eviral.chat_id, guser.id, view_messages=False
                            )
                            await eviral.reply(
