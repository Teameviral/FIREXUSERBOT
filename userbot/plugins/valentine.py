...
from FIREX import ALIVE_NAME, CmdHelp
...
@FIREX.on(admin_cmd(pattern="hvd$"))
@FIREX.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@FIREX.on(admin_cmd(pattern="vday$"))
@FIREX.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
