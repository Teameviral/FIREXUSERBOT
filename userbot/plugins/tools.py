...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    eviralevent = await edit_or_reply(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eviralevent.edit("`Please unblock `@DrWebBot `and try again`")
            return
...
async def _(event):
    if event.fwd_from:
        return
    eviralevent = await edit_or_reply(event, "...")
    start = datetime.now()
    ...
    try:
        ...
        await eviralevent.edit(str(e))
        return
    end = datetime.now()
    ms = (end - start).seconds
    await eviralevent.edit("Created BarCode in {} seconds🤓".format(ms))
...
async def _(event):
    if event.fwd_from:
        return
    eviralevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await eviralevent.edit(output_str, link_preview=True)
    else:
        await eviralevent.edit("xkcd n.{} not found!".format(xkcd_id))
