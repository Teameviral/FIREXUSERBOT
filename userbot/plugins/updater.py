...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"🌚 Do `.update build` to update your **eviralẞø†** !!"
        )
        ...
        if len(heroku_applications) >= 1:
            if Var.HEROKU_APP_NAME is not None:
                ...
                asyncio.get_event_loop().create_task(
                    deploy_start(firebot, message, HEROKU_GIT_REF_SPEC, remote)
                )
...
"""async def deploy_start(firebot, message, refspec, remote):
    await message.edit(RESTARTING_APP)
    await message.edit(
        "Updating Userbot In Progress...Please wait upto 5 minutes."
    )
    await remote.push(refspec=refspec)
    await firebot.disconnect()
