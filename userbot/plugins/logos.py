...
async def lg1(eviralevent):
    event = await edit_or_reply(eviralevent, "`Processing.....`")
    fnt = await get_font_file(event.client, "@eviralFonts")
    if eviralevent.reply_to_msg_id:
        rply = await eviralevent.get_reply_message()
        logo_ = await rply.download_media()
    else:
        async for i in bot.iter_messages(
            "@eviral_MR_LOGOS", filter=InputMessagesFilterPhotos
        ):
            PICS_STR.append(i)
        pic = random.choice(PICS_STR)
        logo_ = await pic.download_media()
    text = eviralevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
