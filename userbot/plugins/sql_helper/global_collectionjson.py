...
class eviral_GlobalCollection_Json(BASE):
    __tablename__ = "eviral_globalcollectionjson"
...
eviral_GlobalCollection_Json.__table__.create(checkfirst=True)


def get_collection(keywoard):
    try:
        return SESSION.query(eviral_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
