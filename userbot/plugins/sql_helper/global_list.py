...
class eviralGloballist(BASE):
    __tablename__ = "eviralglobal_list"
    ...
    def __repr__(self):
        return "<eviral global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, eviralGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


eviralGloballist.__table__.create(checkfirst=True)

eviralGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with eviralGLOBALLIST_INSERTION_LOCK:
        broadcast_group = eviralGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with eviralGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(eviralGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with eviralGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(eviralGloballist.keywoard)
            .filter(eviralGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(eviralGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(eviralGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(eviralGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(eviralGloballist).all()
        for x in all_groups:
