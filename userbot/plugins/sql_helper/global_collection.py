...
class eviral_GlobalCollection(BASE):
    __tablename__ = "eviral_globalcollection"
    ...
    def __repr__(self):
        return "<eviral Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, eviral_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


eviral_GlobalCollection.__table__.create(checkfirst=True)

eviral_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with eviral_GLOBALCOLLECTION:
        keyword_items = eviral_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with eviral_GLOBALCOLLECTION:
        keyword_items = SESSION.query(eviral_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with eviral_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(eviral_GlobalCollection.keywoard)
            .filter(eviral_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(eviral_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(eviral_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(eviral_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(eviral_GlobalCollection).all()
        for x in all_groups:
