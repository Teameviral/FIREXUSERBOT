...
async def remove_background(event):
    ...
    if event.reply_to_msg_id and not input_str:
        ...
        eviralevent = await edit_or_reply(event, "`Analysing...`")
        file_name = os.path.join(TEMP_DIR, "rmbg.png")
        try:
            await event.client.download_media(reply_message, file_name)
        except Exception as e:
            await edit_or_reply(eviralevent, f"`{str(e)}`")
            return
        else:
            await eviralevent.edit("`Removing Background of this media`")
            file_name = convert_toimage(file_name)
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif input_str:
        eviralevent = await edit_or_reply(event, "`Removing Background of this media`")
    ...
    remove_bg_image = "FIREX.png"
    if "image" in contentType:
        with open("FIREX.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await edit_or_reply(eviralevent, f"`{response.content.decode('UTF-8')}`")
        return
    if cmd == "srmbg":
        file = convert_tosticker(remove_bg_image, filename="FIREX.webp")
